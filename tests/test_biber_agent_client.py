...
def test_run_local_repair_chain_extracts_without_api_key(
    monkeypatch,
    tmp_path: Path,
) -> None:
    def fake_resolve_api_key(cli_api_key: str | None = None) -> str:
        raise AssertionError("local-repair-chain should not resolve an API key")
    ...
    output_path = tmp_path / "local-repair-chain.json"
    ...
    assert saved == result
    assert result["source"] == "biber_local_repair_chain"
    assert result["chain_status"] == "extracted"
    assert result["ok"] is True
    assert result["training_allowed"] is False
    assert result["auto_applied"] is False
    assert result["apply_allowed"] is False
    assert result["mentor_used"] is False
    assert result["plan_skipped_reason"] == "target_root_not_supplied"
    assert result["repair_attempt"]["model_response"]["local_supplied"] is True
    assert result["repair_edit_extraction"]["extraction_status"] == (
        "ready_for_plan_edit"
    )
    assert result["repair_edit_extraction"]["plan_edit_payload"]["edits"][0][
        "path"
    ] == "src/app.py"
    assert "repair_edit_plan" not in result


def test_run_local_repair_chain_plans_with_local_target_without_api_key(
    monkeypatch,
    tmp_path: Path,
) -> None:
    captured: dict[str, object] = {}
    ...
    def fake_resolve_api_key(cli_api_key: str | None = None) -> str:
        raise AssertionError("local-repair-chain plan should not resolve an API key")
    ...
    assert captured["payload"]["edits"][0]["path"] == "src/app.py"
    assert result["chain_status"] == "planned"
    assert result["repair_edit_plan"]["plan_mode"] == "local_target_root"
    assert result["repair_edit_plan"]["target_root"] == str(target_root.resolve())
...
def test_run_export_blocked_retry_edit_gap_writes_review_queue_without_api_key(
    monkeypatch,
    tmp_path: Path,
) -> None:
    def fake_resolve_api_key(cli_api_key: str | None = None) -> str:
        raise AssertionError("export-blocked-retry-edit-gap should not resolve an API key")
        ...
        json.dumps(
            {
                ...
                "repair_attempt_artifact": str(attempt),
    ...
    output_path = tmp_path / "blocked-retry-edit-gap.jsonl"
    monkeypatch.setattr(client, "resolve_api_key", fake_resolve_api_key)
    ...
    assert result["source"] == "biber_mvp_loop_blocked_retry_edit_gap_export"
    assert result["records"] == 1
    assert result["training_allowed"] is False
    assert result["eligible_for_training"] is False
    assert result["safe_to_train"] is False
    assert result["auto_promoted"] is False
    assert result["auto_saved"] is False
    assert result["source_artifact"] == str(review)
    assert result["repair_edit_extraction_artifact"] == str(extraction)
    assert result["repair_attempt_artifact"] == str(attempt)
    assert result["review_hard_blockers"] == [
        "retry_edit_changes_previous_failed_target_outside_rule_context"
    ]
    assert len(rows) == 1
    row = rows[0]
    assert row["source"] == "biber_mvp_loop_blocked_retry_edit_gap"
    assert row["gap_type"] == "blocked_retry_repair_edit_candidate"
    assert row["review_status"] == "needs_human_review"
    assert row["training_allowed"] is False
    assert row["eligible_for_training"] is False
    assert row["safe_to_train"] is False
    assert row["plan_allowed"] is False
    assert row["retry_edit_review_artifact"] == str(review)
    assert row["repair_edit_extraction_artifact"] == str(extraction)
    assert row["repair_attempt_artifact"] == str(attempt)
    assert row["model"] == "biber-dev-core-v1"
    assert row["repair_prompt"] == repair_prompt
...
def test_export_blocked_retry_edit_gap_rejects_allowed_review(
    tmp_path: Path,
) -> None:
    attempt = tmp_path / "retry-attempt.json"
    attempt.write_text(
        ...
        json.dumps(
            {
                ...
                "hard_blockers": [],
                "candidate_reviews": [
                    {"index": 1, "allowed_for_plan": True, "hard_blockers": []}
                ],
            }
        ),
        encoding="utf-8",
    ...
    try:
        ...
        assert "requires a blocked retry edit review" in str(exc)
    else:
...
def test_run_list_ready_repair_chain_reviews_summarizes_without_api_key(
    ...
    list_artifact = tmp_path / "ready-repair-chain-review-list.json"
    json_output = client.run(
        client.parse_args(
            [
                "--json",
    ...
    saved = json.loads(list_artifact.read_text(encoding="utf-8"))
    show_output = client.run(
        client.parse_args(["show-ready-repair-chain-review-list", str(list_artifact)])
    )
    show_json = json.loads(
        client.run(
            client.parse_args(
    ...
    assert result["source"] == "biber_mvp_loop_ready_repair_chain_review_list"
    assert result["artifact_path"] == str(list_artifact)
    assert saved == result
    assert show_json == result
    assert "BIBER ready repair-chain review artifacts (1)" in show_output
    assert str(ready) in show_output
    assert "status=needs_human_review" in show_output
...
def test_run_list_ready_repair_chain_decision_reviews_summarizes_without_api_key(
    ...
    list_artifact = tmp_path / "ready-repair-chain-decision-review-list.json"
    json_output = client.run(
        client.parse_args(
            [
                "--json",
    ...
    show_output = client.run(
        client.parse_args(
            ["show-ready-repair-chain-decision-review-list", str(list_artifact)]
        )
    )
    show_json = json.loads(
        client.run(
    ...
    assert saved == result
    assert show_json == result
    assert "BIBER ready repair-chain decision review artifacts (1)" in show_output
    assert str(defer_review) in show_output
    assert "decision: defer" in show_output
    assert "defer=1" in show_output
...
def test_run_list_ready_repair_chain_eval_dataset_decision_reviews_without_api_key(
    ...
    list_artifact = (
        tmp_path / "ready-repair-chain-eval-dataset-decision-review-list.json"
    )
    json_output = client.run(
        client.parse_args(
            [
    ...
    assert str(ready_artifact) in show_output
    assert "decision: approve_for_eval_dataset" in show_output
    assert "ready_only: True" in show_output
...
def test_run_list_ready_repair_chain_eval_prompts_without_api_key(
    ...
    list_artifact = tmp_path / "ready-repair-chain-eval-prompt-list.json"
    json_output = client.run(
        client.parse_args(
            [
                "--json",
    ...
    saved = json.loads(list_artifact.read_text(encoding="utf-8"))
    show_output = client.run(
        client.parse_args(["show-ready-repair-chain-eval-prompt-list", str(list_artifact)])
    )
    show_json = json.loads(
        client.run(
            client.parse_args(
    ...
    assert result["source"] == "biber_mvp_loop_ready_repair_chain_eval_prompt_list"
    assert result["artifact_path"] == str(list_artifact)
    assert saved == result
    assert show_json == result
    assert "BIBER ready repair-chain eval prompt artifacts (1)" in show_output
    assert str(ready_artifact) in show_output
    assert "ready_only: True" in show_output
    assert "ok=True" in show_output
