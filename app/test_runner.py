...
def list_test_commands(settings: Settings) -> list[dict[str, object]]:
    return [
        {
...
def _cwd(spec: TestCommandSpec, settings: Settings) -> Path:
    if spec.cwd == "repo":
...
def _display_cwd(spec: TestCommandSpec, settings: Settings) -> str:
    try:
...
def _timeout_seconds(spec: TestCommandSpec, settings: Settings) -> float:
