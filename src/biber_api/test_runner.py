...
_TEST_COMMANDS: dict[str, TestCommandSpec] = {
    ...
    "python-pytest": TestCommandSpec(
        test_id="python-pytest",
        label="Python pytest",
        description="Run `python -m pytest -q` from the configured repo root.",
        cwd="repo",
    ...
    "cargo-test": TestCommandSpec(
        test_id="cargo-test",
        label="Cargo test",
        description="Run `cargo test` from the configured repo root.",
        cwd="repo",
        argv=("cargo", "test"),
        timeout_seconds=300,
    ),
    "npm-run-check": TestCommandSpec(
        test_id="npm-run-check",
        label="npm check",
        description="Run `npm run check` from the configured repo root.",
        cwd="repo",
        argv=("npm", "run", "check"),
        timeout_seconds=300,
    ),
    "npm-test": TestCommandSpec(
        test_id="npm-test",
        label="npm test",
        description="Run `npm test` from the configured repo root.",
        cwd="repo",
        argv=("npm", "test"),
        timeout_seconds=300,
    ),
    "npm-run-build": TestCommandSpec(
        test_id="npm-run-build",
        label="npm build",
        description="Run `npm run build` from the configured repo root.",
        cwd="repo",
        argv=("npm", "run", "build"),
        timeout_seconds=300,
    ),
    "docker-compose-config": TestCommandSpec(
        test_id="docker-compose-config",
        label="Docker Compose config",
        description="Run `docker compose config` from the configured repo root.",
        cwd="repo",
