...
`xriq/fixtures/phase1_3/local-wallet-behavior-v1.json`; validate it with
`python scripts/xriq_phase1_3_behavior_contract_check.py` or the bundled Codex
Python if `python` is not on PATH.
...
Generate/check the negative scenario artifacts with
...
`docs/XRIQ_PHASE1_4_RC_CANDIDATE_REPORT.md`; validate its readiness/status
guardrails with `python scripts/xriq_phase1_4_rc_readiness.py`. The approved
RC tag is `phase1-4-xriq-local-signed-submit-rc1` at commit `45be474`; do not
move, delete, recreate, or repush it without an exact tag-maintenance request.
The consolidated XRIQ private-devnet wrap-up is in
`docs/XRIQ_PRIVATE_DEVNET_WRAPUP.md`; validate the 100% non-production
private-devnet handoff with
...
The Phase 2 hardened private/staging-devnet planning checkpoint is in
...
A clean clone can build and run the Phase 2 staging smokes (lifecycle plus
restart/recovery under the fail-closed `staging-devnet` environment profile) with
...
enable wallet submit/send behavior. Validate them with
...
`xriq_phase1_1_smoke` database schema, applies the generated write plan,
imports the generated durable pending TSV row into `xriq_mempool_entries`, and
...
`docs/XRIQ_PHASE1_1_RC_READINESS.md`. After a Docker live smoke, run the cheap
guardrail from the repo root:
...
ISO 20022 transaction status, ISO 20022 payment initiation, ISO 20022 account statement, account list, wallet
