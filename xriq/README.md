...
The UI-backed Phase 1.3 shared-client behavior smoke is
`../scripts/xriq_phase1_3_wallet_behavior_ui_smoke.py`; from the repo root, run
it with `python scripts/xriq_phase1_3_wallet_behavior_ui_smoke.py --skip-build`
after local `xriq-node` and `xriq-api` debug binaries exist.
The Phase 1.3 local/private behavior readiness and negative-matrix
consolidation guard is `../scripts/xriq_phase1_3_readiness_summary.py`; from the
repo root, run it with `python scripts/xriq_phase1_3_readiness_summary.py`.
...
Generate/check the negative scenario artifacts from the repo root with
...
`--enable-local-wallet-submit-signed true`; from this directory, check
...
The Phase 1.4 signed-submit lifecycle smoke can be run from the repo root with
