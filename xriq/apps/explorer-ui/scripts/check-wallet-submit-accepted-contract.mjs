...
const fixturePath =
  args["--fixture"] ??
  join(repoRoot, "xriq/fixtures/phase1_2/wallet-transfer-submit-to-pending-contract.json");
const artifactPath = args["--artifact"] ?? latestWalletSubmitArtifact();
...
const summary = {
  ok: "xriq-wallet-submit-accepted-client-contract",
  fixture: fixturePath,
  fixture_checked: true,
  artifact: artifactPath,
  artifact_checked: Boolean(artifactPath),
  validator: "validateLocalWalletSubmitAcceptedContract",
};
...
function validateAcceptedResponse(apiModule, payload, label, expectations) {
  ...
  const validatorErrors = apiModule.validateLocalWalletSubmitAcceptedContract(
    payload,
    expectations,
  );
...
function latestWalletSubmitArtifact() {
  const targetDir = join(repoRoot, "xriq/target");
  if (!existsSync(targetDir)) {
    return null;
  }
  const candidateDirs = readdirSync(targetDir)
    .filter((name) => name.startsWith("xriq-phase1-2-wallet-submit-smoke-"))
    .sort()
    .reverse();
  for (const dir of candidateDirs) {
    const artifact = join(targetDir, dir, "api/wallet-submit-accepted-local.json");
