...
- Phase 1.1 estimated completion: about `77%` overall. Current Rust
  ...
  overview/block/transaction-list/transaction-detail/account-list/account-detail/account-history/wallet-account-history
...
- Latest native XRIQ Phase 1.1 Postgres-backed wallet account history checkpoint:
...
- Recommended next narrow step: add the next read-only Postgres-backed product
  route, preferably a wallet account list parity route for
  `/api/v1/wallet/accounts?limit=...` or a small wallet balance route after
  confirming UI needs. Preserve default file-backed behavior and keep all
  Postgres use opt-in.
