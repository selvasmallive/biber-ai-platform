...
from the latest real-repo plan smoke. This approval covers only the guarded
real-repo apply for artifacts under
`/workspace/outputs/biber-real-repo-plan-smoke-20260728T094239Z/artifacts`.
...
or use GPU.
Twenty-fifth BIBER resume checkpoint on the same branch:
failed MVP-loop artifacts now include `agent_report.repair_hint`, a compact
CPU-local repair packet with the failed test id, command, exit/timing fields,
diagnosis category, detected stack, relevant output preview, suggested next
actions, and the no-API local repair workflow. `show-mvp-loop` surfaces the
hint, `export-mvp-failures` writes it into review JSONL, and `prepare-repair`
propagates or derives it for older failed artifacts. Future sessions should
...
does not call OpenAI mentor, train, or use GPU.
Twenty-seventh BIBER resume checkpoint on the same branch:
`local-repair-loop-status` now surfaces repair-hint metadata for failed
MVP-loop and prepared repair artifacts: hint status, primary category, detected
stack, and the first local repair workflow steps. Future sessions can inspect
...
Forty-ninth BIBER resume checkpoint on the same branch:
`scripts/biber_local_mvp_loop_failure_smoke.py` now verifies the new
`list-mvp-loops --failed-only` repair-hint fields against a real saved failed
MVP-loop artifact, and `scripts/biber_local_confidence_smoke.py` carries the
list proof in its `mvp_loop_failure` summary (`list_failed_artifacts`,
`list_repair_hint_status`, and `list_repair_next_step`). The standard no-GPU
...
OpenAI mentor, external paid APIs, or credential rotation.
Fifty-second BIBER resume checkpoint on the same branch:
`scripts/biber_agent_client.py list-repair-attempts` now supports `--output`
and writes saved repair-attempt queue JSON with
`source=biber_mvp_loop_repair_attempt_list` plus `artifact_path`.
`show-repair-attempt-list <artifact>` summarizes that saved queue without
rerunning the directory scan. This is the next resumability step after
`prepare-repair` / `attempt-repair`, and remains CPU-local with no GPU,
training, OpenAI mentor, external paid API, or credential rotation.
Fifty-third BIBER resume checkpoint on the same branch:
`scripts/biber_agent_client.py list-repair-edit-extractions` now supports
`--output` and writes saved extraction queue JSON with
`source=biber_mvp_loop_repair_edit_extraction_list` plus `artifact_path`.
`show-repair-edit-extraction-list <artifact>` summarizes that saved queue
without rerunning the directory scan. This is the next resumability step before
`plan-repair-edits`, and remains CPU-local with no GPU, training, OpenAI
mentor, external paid API, or credential rotation.
Fifty-fourth BIBER resume checkpoint on the same branch:
`scripts/biber_agent_client.py list-repair-edit-plans` now supports `--output`
and writes saved repair-plan queue JSON with
`source=biber_mvp_loop_repair_edit_plan_list` plus `artifact_path`.
`show-repair-edit-plan-list <artifact>` summarizes that saved planned queue
without rerunning the directory scan. This is the next resumability step before
...
rotation.
Fifty-seventh BIBER resume checkpoint on the same branch:
`scripts/biber_agent_client.py list-verified-repair-reviews` now supports
`--output` and writes saved human-review queue JSON with
`source=biber_mvp_loop_verified_repair_review_list` plus `artifact_path`.
`show-verified-repair-review-list <artifact>` summarizes that saved review
...
credential rotation.
Fifty-ninth BIBER resume checkpoint on the same branch:
`scripts/biber_agent_client.py list-ready-repair-chain-reviews` now supports
`--output` and writes saved ready-review queue JSON with
`source=biber_mvp_loop_ready_repair_chain_review_list` plus `artifact_path`.
`show-ready-repair-chain-review-list <artifact>` summarizes that saved queue
without rerunning the directory scan. This is the next resumability step before
`record-ready-repair-chain-decision`, and remains CPU-local with no GPU,
training, OpenAI mentor, external paid API, or credential rotation.
Sixtieth BIBER resume checkpoint on the same branch:
`scripts/biber_agent_client.py list-ready-repair-chain-decision-reviews` now
supports `--output` and writes saved decision-review queue JSON with
`source=biber_mvp_loop_ready_repair_chain_decision_review_list` plus
`artifact_path`. `show-ready-repair-chain-decision-review-list <artifact>`
summarizes that saved queue without rerunning the directory scan. This is the
next resumability step before `export-ready-repair-chain-eval-candidates`, and
remains CPU-local with no GPU, training, OpenAI mentor, external paid API, or
credential rotation.
Sixty-first BIBER resume checkpoint on the same branch:
`scripts/biber_agent_client.py list-ready-repair-chain-eval-candidate-reviews`
now supports `--output` and writes saved eval-candidate review queue JSON with
`source=biber_mvp_loop_ready_repair_chain_eval_candidate_review_list` plus
`artifact_path`. `show-ready-repair-chain-eval-candidate-review-list <artifact>`
summarizes that saved queue without rerunning the directory scan. This is the
...
remains CPU-local with no GPU, training, OpenAI mentor, external paid API, or
credential rotation.
Sixty-third BIBER resume checkpoint on the same branch:
`scripts/biber_agent_client.py list-ready-repair-chain-eval-dataset-validations`
now supports `--output` and writes saved eval-dataset validation queue JSON with
`source=biber_mvp_loop_ready_repair_chain_eval_dataset_validation_list` plus
`artifact_path`. `show-ready-repair-chain-eval-dataset-validation-list <artifact>`
summarizes that saved queue without rerunning the directory scan. This is the
