...
`xriq/crates/xriq-indexer`, and it now has a local replay command; it still
...
exists. The current `xriq-indexer` crate and replay command are intentionally
in-memory, so they do not justify Cloud SQL yet.
