...
Rust
 ...
 |-- APIs                   ~81%; local HTTP wrappers plus wallet/admin/node/pending-file mempool/pending-status/ISO preview read routes and explicit Postgres read-model status, node status, indexer status, explorer overview, block list/detail, transaction list, mempool, wallet status, wallet draft-preview, transaction detail, confirmed/pending wallet transaction status, ISO 20022 transaction status, account list, wallet account list, account detail, wallet balance, account history, wallet account history, audit-events, snapshot-list, and snapshot-detail CLI/server paths exist
...
SQL/PostgreSQL
 |-- Explorer indexing      ~65%; schema, indexer, SQL plan, verify path, Docker live smoke, Postgres-backed API/server read-model/node/indexer status reads, and first Postgres-backed product overview/block-list/block-detail/transaction list/mempool/wallet-status/wallet-draft-preview/detail/confirmed-and-pending-wallet-status/ISO-status/ISO-initiation/ISO-statement/account/wallet-account/wallet-balance/history/wallet-history/audit-events/snapshot-list/snapshot-detail reads exist
...
ISO 20022
 `-- Compatibility adapter  ~34%; preview crate, product API read routes, UI panel, and first opt-in Postgres status/initiation/statement routes exist
```
...
After the local Phase 1.1 RC candidate report checkpoint, Phase 1.1 status
is about `95%`: the contract document, PostgreSQL read-model schema, JSON
...
route plus `/api/v1/admin/node/status` and `/api/v1/admin/indexer/status`, the first opt-in Postgres-backed product data routes for
...
generated write plan through container-local `psql`, imports the local durable
pending TSV row into `xriq_mempool_entries`, verifies indexed counts,
calls `xriq-api request-postgres`, and writes
`indexer/postgres-docker-live.json` plus
`indexer/postgres-api-read-model-status.json` and
`indexer/postgres-api-node-status.json` plus
`indexer/postgres-api-indexer-status.json` plus
`indexer/postgres-api-explorer-overview.json` plus
`indexer/postgres-api-blocks.json` plus
...
`indexer/postgres-api-iso-account-statement.json` plus
...
`indexer/postgres-api-account-detail.json` plus
`indexer/postgres-api-wallet-balance.json` plus
...
`indexer/postgres-server-explorer-overview.json` plus
`indexer/postgres-server-blocks.json` plus
...
`indexer/postgres-server-iso-account-statement.json` plus
...
`indexer/postgres-server-account-detail.json` plus
`indexer/postgres-server-wallet-balance.json` plus
...
readiness guardrail after a Docker live smoke. The candidate report for the RC
decision is tracked in `docs/XRIQ_PHASE1_1_RC_CANDIDATE_REPORT.md`.
...
- Current artifacts: `docs/XRIQ_PHASE1_1_CONTRACTS.md`,
  ...
  `docs/XRIQ_PHASE1_1_RC_CANDIDATE_REPORT.md`, `xriq/db/schema.sql`,
  `xriq/fixtures/phase1_1/`, `scripts/xriq_phase1_1_contract_check.py`,
  and `scripts/xriq_phase1_1_rc_readiness.py`.
...
- Current scaffold: `xriq/crates/xriq-api` exposes read-only private-devnet
  ...
  ISO 20022 preview routes, and explicit local Docker Postgres read-model
  ...
  `/api/v1/wallet/accounts?limit=...`,
  `/api/v1/wallet/accounts/{address}/balance`,
  `/api/v1/wallet/accounts/{address}/history?limit=...`, a
  `request` CLI smoke path, and a local `serve-readonly` socket wrapper. The pending-file path does not enable
  product API submit or block production. Focused verification is:
