...
After the first local Phase 1.1 indexer dry-run smoke checkpoint, Phase 1.1
status is about `63%`: the contract document, PostgreSQL read-model schema, JSON
...
artifact, and validates apply/verify PostgreSQL dry-runs without requiring a
live database. `xriq-api` now calls the ISO
...
preview surfaces without opening a socket or using cloud/GPU resources.
...
- Current scaffold: `xriq/crates/xriq-indexer` builds the PostgreSQL-facing
  ...
  the chain generated by the smoke:
