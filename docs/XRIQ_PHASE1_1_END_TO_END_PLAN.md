...
Rust
 ...
 |-- APIs                   ~61%; local HTTP wrappers plus wallet/admin/node/pending-file mempool/pending-status/ISO preview read routes and explicit Postgres read-model status, explorer overview, and blocks CLI/server paths exist
...
SQL/PostgreSQL
 |-- Explorer indexing      ~37%; schema, indexer, SQL plan, verify path, Docker live smoke, Postgres-backed API/server status read, and first Postgres-backed product overview/blocks reads exist
...
After the first local Phase 1.1 Postgres-backed blocks checkpoint,
Phase 1.1 status is about `71%`: the contract document, PostgreSQL read-model schema, JSON
...
`/api/v1/explorer/overview` and `/api/v1/blocks?limit=...`, the `xriq-api` read-only product response boundary
...
`indexer/postgres-api-blocks.json`. The same live mode now starts a
