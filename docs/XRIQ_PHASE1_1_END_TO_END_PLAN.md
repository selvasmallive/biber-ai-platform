...
Rust
 ...
 |-- APIs                   ~68%; local HTTP wrappers plus wallet/admin/node/pending-file mempool/pending-status/ISO preview read routes and explicit Postgres read-model status, explorer overview, blocks, transaction list, transaction detail, account list, wallet account list, account detail, account history, and wallet account history CLI/server paths exist
...
SQL/PostgreSQL
 |-- Explorer indexing      ~45%; schema, indexer, SQL plan, verify path, Docker live smoke, Postgres-backed API/server status read, and first Postgres-backed product overview/blocks/transaction list/detail/account/wallet-account/history/wallet-history reads exist
...
After the first local Phase 1.1 Postgres-backed wallet account-list checkpoint,
Phase 1.1 status is about `78%`: the contract document, PostgreSQL read-model schema, JSON
