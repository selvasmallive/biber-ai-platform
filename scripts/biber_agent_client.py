...
def normalize_repair_edit_plan_artifact_list(
    payload: Mapping[str, Any],
) -> dict[str, Any] | None:
    if payload.get("source") == "biber_mvp_loop_repair_edit_plan_list":
        return dict(payload)
    if isinstance(payload.get("artifacts"), list) and (
        payload.get("directory") or payload.get("pattern")
    ):
    ...
    body = payload.get("body")
    if isinstance(body, dict):
        return normalize_repair_edit_plan_artifact_list(body)
    return None
...
def normalize_ready_repair_chain_review_artifact_list(
    payload: Mapping[str, Any],
) -> dict[str, Any] | None:
    if payload.get("source") == "biber_mvp_loop_ready_repair_chain_review_list":
        return dict(payload)
    if isinstance(payload.get("artifacts"), list) and (
        payload.get("directory") or payload.get("pattern")
    ):
    ...
    body = payload.get("body")
    if isinstance(body, dict):
        return normalize_ready_repair_chain_review_artifact_list(body)
    return None
...
def normalize_ready_repair_chain_eval_candidate_review_artifact_list(
    payload: Mapping[str, Any],
) -> dict[str, Any] | None:
    if (
        payload.get("source")
    ...
    body = payload.get("body")
    if isinstance(body, dict):
        return normalize_ready_repair_chain_eval_candidate_review_artifact_list(body)
    return None
...
def normalize_ready_repair_chain_eval_dataset_decision_review_artifact_list(
    payload: Mapping[str, Any],
) -> dict[str, Any] | None:
    if (
        payload.get("source")
        == "biber_mvp_loop_ready_repair_chain_eval_dataset_decision_review_list"
    ):
        return dict(payload)
    if isinstance(payload.get("artifacts"), list) and (
        payload.get("directory") or payload.get("pattern")
    ...
    body = payload.get("body")
    if isinstance(body, dict):
        return normalize_ready_repair_chain_eval_dataset_decision_review_artifact_list(
            body
        )
    return None
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    show_repair_edit_plan_list = subparsers.add_parser(
        "show-repair-edit-plan-list",
        help="Summarize a saved list-repair-edit-plans --output JSON artifact.",
    )
    show_repair_edit_plan_list.add_argument("artifact")
    ...
    list_repair_edit_plans.add_argument("--output")
    ...
    show_ready_repair_chain_review_list = subparsers.add_parser(
        "show-ready-repair-chain-review-list",
        help=(
            "Summarize a saved list-ready-repair-chain-reviews --output JSON artifact."
        ),
    )
    show_ready_repair_chain_review_list.add_argument("artifact")
    ...
    list_ready_repair_chain_reviews.add_argument("--output")
    ...
    show_ready_repair_chain_eval_candidate_review_list = subparsers.add_parser(
        "show-ready-repair-chain-eval-candidate-review-list",
        help=(
            "Summarize a saved list-ready-repair-chain-eval-candidate-reviews "
            "--output JSON artifact."
        ),
    )
    show_ready_repair_chain_eval_candidate_review_list.add_argument("artifact")
    ...
    list_ready_repair_chain_eval_candidate_reviews.add_argument("--output")
    ...
    show_ready_repair_chain_eval_dataset_decision_review_list = (
        subparsers.add_parser(
            "show-ready-repair-chain-eval-dataset-decision-review-list",
            help=(
                "Summarize a saved "
    ...
    show_ready_repair_chain_eval_dataset_decision_review_list.add_argument("artifact")
    ...
    list_ready_repair_chain_eval_dataset_decision_reviews.add_argument("--output")
...
def run(args: argparse.Namespace) -> str:
    ...
    if args.command == "show-repair-edit-plan-list":
        artifact = load_json_artifact(
            args.artifact,
            label="repair-edit plan list artifact",
        )
        normalized = normalize_repair_edit_plan_artifact_list(artifact)
        if normalized is None:
            raise BiberAgentClientError(
                "repair-edit plan list artifact must contain a saved "
                "list-repair-edit-plans JSON object."
            )
        if not normalized.get("artifact_path"):
            normalized["artifact_path"] = str(Path(args.artifact))
        return (
            json.dumps(normalized, indent=2, sort_keys=True)
            if args.print_json
            else format_repair_edit_plan_artifact_list_summary(normalized)
        )
    ...
    if args.command == "show-ready-repair-chain-review-list":
        artifact = load_json_artifact(
            args.artifact,
            label="ready repair-chain review list artifact",
        )
        normalized = normalize_ready_repair_chain_review_artifact_list(artifact)
        if normalized is None:
            raise BiberAgentClientError(
                "ready repair-chain review list artifact must contain a saved "
                "list-ready-repair-chain-reviews JSON object."
            )
        if not normalized.get("artifact_path"):
            normalized["artifact_path"] = str(Path(args.artifact))
        return (
            json.dumps(normalized, indent=2, sort_keys=True)
            if args.print_json
            else format_ready_repair_chain_review_artifact_list_summary(normalized)
        )
    ...
    if args.command == "show-ready-repair-chain-eval-candidate-review-list":
        artifact = load_json_artifact(
            args.artifact,
            label="ready repair-chain eval-candidate review list artifact",
        )
        normalized = normalize_ready_repair_chain_eval_candidate_review_artifact_list(
            artifact
        )
        if normalized is None:
            raise BiberAgentClientError(
                "ready repair-chain eval-candidate review list artifact must "
                "contain a saved list-ready-repair-chain-eval-candidate-reviews "
                "JSON object."
            )
        if not normalized.get("artifact_path"):
            normalized["artifact_path"] = str(Path(args.artifact))
    ...
    if args.command == "show-ready-repair-chain-eval-dataset-decision-review-list":
        artifact = load_json_artifact(
            args.artifact,
            label="ready repair-chain eval-dataset decision review list artifact",
        )
        normalized = (
            normalize_ready_repair_chain_eval_dataset_decision_review_artifact_list(
                artifact
            )
        )
        if normalized is None:
            raise BiberAgentClientError(
                "ready repair-chain eval-dataset decision review list artifact "
                "must contain a saved "
                "list-ready-repair-chain-eval-dataset-decision-reviews JSON "
                "object."
            )
        if not normalized.get("artifact_path"):
            normalized["artifact_path"] = str(Path(args.artifact))
